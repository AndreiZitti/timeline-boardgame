 ...
 * Enables live view sharing via room code for Rentz.
 */
export function useRentzRoom() {
  const { isAuthenticated, user } = useUser();
  const scoreTracker = useScoreTracker('rentz');
    ...
    try {
      ...
      const { data, error } = await supabaseGames
        ...
        .insert({
          ...
          game_config: scoreTracker.rentzConfig,
          game_state: scoreTracker.rentzData,
        })
        .select()
        .single();
  ...
  }, [isAuthenticated, user?.id, scoreTracker.players, scoreTracker.rentzConfig, scoreTracker.rentzData, roomCode]);

  // Sync state to Supabase whenever rentzData changes
  useEffect(() => {
    if (!roomCode || !isAuthenticated) return;
    if (scoreTracker.rentzData.length === 0) return;
    ...
    syncState();
  }, [roomCode, isAuthenticated, scoreTracker.rentzData, scoreTracker.players]);
  ...
  const shareUrl = roomCode
    ? `${typeof window !== 'undefined' ? window.location.origin : ''}/score-tracker/rentz/view/${roomCode}`
    : null;
